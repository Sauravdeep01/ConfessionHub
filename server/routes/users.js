...
router.post('/change-password', async (req, res) => {
    try {
        ...
        // Look for user by either googleId or customUserId
        const user = await User.findOne({
            $or: [{ googleId: userId }, { customUserId: userId }]
        });
        ...
        // Check if user has password (Google-only users can't change password)
        if (!user.password) {
            return res.status(400).json({ error: 'This account uses Google Sign-In only' });
        }
